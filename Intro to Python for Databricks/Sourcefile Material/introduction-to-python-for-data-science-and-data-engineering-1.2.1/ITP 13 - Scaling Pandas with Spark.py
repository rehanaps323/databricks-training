...
# MAGIC * If workloads are not working correctly or not delivering expected results, then use **distributed-squence** for best compatibility while sacrificing some performance.
...
df_abc = ps.sql("SELECT distinct(property_type) FROM {df}", df=df).
print(df_abc) 
